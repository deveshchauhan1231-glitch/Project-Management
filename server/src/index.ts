...
const logActivity = async (projectId: string | null, message: string, actor = 'You') => {
...
app.delete('/api/tasks/:id', async (req, res) => {
  ...
  if (result.rowCount) await logActivity(result.rows[0].project_id, `Task ${result.rows[0].title} was deleted`, actor);
  res.status(204).send();
});
